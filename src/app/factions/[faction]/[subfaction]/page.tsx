...
export function generateStaticParams() {
  const params: { faction: string; subfaction: string }[] = [];
  for (const gf of GRAND_FACTIONS) {
    if (gf.comingSoon) continue;
    for (const sfId of gf.subfactionIds) {
      params.push({ faction: gf.id, subfaction: sfId });
    }
  }
  return params;
}
...
export default async function SubfactionPage({
  params,
}: {
  params: Promise<{ faction: string; subfaction: string }>;
}) {
  const { faction: gfId, subfaction: sfId } = await params;
  const gf = getGrandFaction(gfId);
  if (!gf || gf.comingSoon) notFound();
  if (!gf.subfactionIds.includes(sfId)) notFound();

  const faction = getFaction(sfId);
  if (!faction) notFound();
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-5xl px-4 pb-12 pt-8">
          ...
          <nav className="mb-8 flex items-center gap-1.5 text-sm animate-fade-in">
            ...
            <span style={{ color: primary }}>{faction.name}</span>
          </nav>
          ...
          <div className="flex items-end gap-5 animate-fade-in-up">
            ...
            <div>
              ...
              >
                {gf.name}
                {isBase ? " — Base Faction" : " — Subfaction"}
              </p>
      ...
      <div className="mx-auto max-w-5xl px-4 py-8">
        ...
        {/* Base faction reminder for non-base subfactions */}
        {!isBase && baseFaction && (
          <div
            className="mb-6 rounded-xl border border-border bg-surface/50 p-4 animate-fade-in-up animate-delay-200"
          >
            <p className="text-xs text-muted">
              As a {gf.name} subfaction, you also have access to{" "}
              <Link
                href={`/factions/${gf.id}/${baseFaction.id}`}
                className="font-semibold transition-colors hover:text-foreground"
                style={{ color: gfColor }}
              >
                {baseFaction.name}
              </Link>{" "}
              base battleline and the{" "}
              <strong className="text-foreground">
                {baseFaction.mechanic.name}
              </strong>{" "}
              faction mechanic.
            </p>
          </div>
        ...
        <div className="mb-8 grid gap-4 md:grid-cols-3 animate-fade-in-up animate-delay-300">
          <div className="rounded-xl border border-border bg-surface p-4">
            ...
            <div className="space-y-1.5">
              <StarRating
                label="Offense"
                value={faction.ratings.offense}
                color={accent}
              />
              <StarRating
                label="Defense"
                value={faction.ratings.defense}
                color={accent}
              />
              <StarRating
                label="Magic"
                value={faction.ratings.magic}
                color={accent}
              />
              <StarRating
                label="Speed"
                value={faction.ratings.speed}
                color={accent}
              />
            </div>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-surface">
                  {["Battle Size", "Baseline", "Mounted", "Elite", "Hero"].map(
                    (col) => (
                      <th
                        key={col}
                ...
                {faction.composition.map((row, i) => (
                  <tr
                    ...
                    style={
                      i % 2 === 1
                        ? {
                            backgroundColor: `color-mix(in srgb, ${primary} 4%, transparent)`,
                          }
                        : undefined
                    }
                  >
